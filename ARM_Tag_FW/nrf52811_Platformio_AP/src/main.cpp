...
{
  Serial.write(data); // uartWrite((uint8_t *)&data, 1);
}
...
{
  struct MacFcs *fcs = (struct MacFcs *)buffer;
...
void processBlockRequest(const uint8_t *buffer, uint8_t forceBlockDownload)
{
  ...
  if (blockStartTimer == 0)
  {
    if (requestDataDownload)
    {
      ...
      {
        blockRequestAck->pleaseWaitMs = 500;
      }
      else
      {
        blockRequestAck->pleaseWaitMs = 500;
