  ...
  for (const element of tagArray) {
    ...
    if (element.RSSI) {
      ...
      if (element.batteryMv != 0 && element.batteryMv != 1337) {
        statusline +=
          ", " +
          (element.batteryMv == 2600 ? "&#x2265;" : "") +
          element.batteryMv / 1000 +
          "V";
      }
    ...
    if (
      !apConfig.preview ||
      element.contentMode == 20 ||
      (element.isexternal && element.contentMode == 12)
    ) {
    ...
    if (element.nextupdate > 1672531200 && element.nextupdate != 3216153600) {
      ...
      $("#tag" + tagmac + " .nextupdate").innerHTML =
        "<span>next update</span>" + date.toLocaleString("nl-NL", options);
    } else {
      $("#tag" + tagmac + " .nextupdate").innerHTML = "";
    }
    ...
    if (element.contentMode == 12)
      $("#tag" + tagmac).style.background = "#e4e4e0";
    switch (parseInt(element.wakeupReason)) {
      ...
      case WAKEUP_REASON_FIRSTBOOT:
        $("#tag" + tagmac + " .nextcheckin").innerHTML =
          "<font color=yellow>First boot</font>";
      ...
      case WAKEUP_REASON_NETWORK_SCAN:
        $("#tag" + tagmac + " .nextcheckin").innerHTML =
          "<font color=yellow>Network scan</font>";
    ...
    $("#tag" + tagmac + " .pendingicon").style.display = element.pending
      ? "inline-block"
      : "none";
    div.classList.add("tagflash");
    (function (tagmac) {
      setTimeout(function () {
        $("#tag" + tagmac).classList.remove("tagflash");
      }, 1400);
    })(tagmac);
...
function updatecards() {
  ...
  $("#taglist")
    .querySelectorAll("[data-mac]")
    .forEach((item) => {
      let tagmac = item.dataset.mac;

      if (
        item.dataset.lastseen &&
        item.dataset.lastseen >
          Date.now() / 1000 - servertimediff - 30 * 24 * 3600 * 60
      ) {
        let idletime =
          Date.now() / 1000 - servertimediff - item.dataset.lastseen;
        $("#tag" + tagmac + " .lastseen").innerHTML =
          "<span>last seen</span>" + displayTime(Math.floor(idletime)) + " ago";
        if (
          Date.now() / 1000 - servertimediff - 600 >
          item.dataset.nextcheckin
        ) {
      ...
      if (
        item.dataset.nextcheckin > 1672531200 &&
        parseInt(item.dataset.wakeupreason) == 0
      ) {
        let nextcheckin =
          item.dataset.nextcheckin - (Date.now() / 1000 - servertimediff);
        $("#tag" + tagmac + " .nextcheckin").innerHTML =
          "<span>expected checkin</span>" +
          displayTime(Math.floor(nextcheckin));
      } else {
        $("#tag" + tagmac + " .nextcheckin").innerHTML = "";
      }
    });
...
$("#cfgmore").onclick = function () {
  $("#cfgmore").innerHTML =
    $("#advancedoptions").style.height == "0px" ? "&#x25B2;" : "&#x25BC;";
  $("#advancedoptions").style.height =
    $("#advancedoptions").style.height == "0px"
      ? $("#advancedoptions").scrollHeight + "px"
      : "0px";
};
...
$("#paintbutton").onclick = function () {
  if (paintShow) {
    ...
    $("#customoptions").innerHTML =
      '<div id="buttonbar"></div><div id="canvasdiv"></div><div id="layersdiv"></div><p id="savebar"></p>';
    const mac = $("#cfgmac").dataset.mac;
    const hwtype = $("#tag" + mac).dataset.hwtype;
    const [width, height] = [
      tagTypes[hwtype].width,
      tagTypes[hwtype].height,
    ] || [0, 0];
    if (paintLoaded) {
...
function contentselected() {
  ...
  let obj = {};
  if (
    $("#cfgcontent").dataset.json &&
    $("#cfgcontent").dataset.json != "null"
  ) {
  ...
  if (contentMode) {
    ...
    $("#paintbutton").style.display =
      contentMode == 0 ? "inline-block" : "none";
    let extraoptions = contentDef?.param ?? null;
    extraoptions?.forEach((element) => {
...
function populateSelectTag(hwtype, capabilities) {
  ...
  cardconfig.forEach((item) => {
    ...
    if (
      (hwtypeArray.includes(hwtype) ||
        tagTypes[hwtype].contentids.includes(item.id)) &&
      (capabilities & capcheck || capcheck == 0)
    ) {
...
function showMessage(message, iserr) {
  ...
  if (iserr) {
    messages.insertAdjacentHTML(
      "afterbegin",
      '<li class="new error">' + htmlEncode(time + " " + message) + "</li>"
    );
  } else {
    messages.insertAdjacentHTML(
      "afterbegin",
      '<li class="new">' + htmlEncode(time + " " + message) + "</li>"
    );
  }
}
...
function processQueue() {
  ...
  fetch(imageSrc, { cache: "force-cache" })
    ...
    .then((buffer) => {
      ...
      if (tagTypes[hwtype].rotatebuffer)
      ...
      if (tagTypes[hwtype].bpp == 16) {
        const is16Bit =
          data.length == tagTypes[hwtype].width * tagTypes[hwtype].height * 2;
        for (
          let i = 0;
          i <
          min(tagTypes[hwtype].width * tagTypes[hwtype].height, data.length);
          i++
        ) {
          const dataIndex = is16Bit ? i * 2 : i;
          const rgb = is16Bit
            ? (data[dataIndex] << 8) | data[dataIndex + 1]
            : data[dataIndex];

          imageData.data[i * 4] = is16Bit
            ? ((rgb >> 11) & 0x1f) << 3
            : (((rgb >> 5) & 0x07) << 5) * 1.13;
          imageData.data[i * 4 + 1] = is16Bit
            ? ((rgb >> 5) & 0x3f) << 2
            : (((rgb >> 2) & 0x07) << 5) * 1.13;
          imageData.data[i * 4 + 2] = is16Bit
            ? (rgb & 0x1f) << 3
            : ((rgb & 0x03) << 6) * 1.3;
        ...
        const offsetRed =
          data.length >= ((canvas.width * canvas.height) / 8) * 2
...
function GroupSortFilter() {
  ...
  const gridItems = Array.from(
    sortableGrid.querySelectorAll(".tagcard:not(#tagtemplate)")
  );

  let grouping = document.querySelector('input[name="group"]:checked')?.value;
  let sorting =
    document.querySelector('input[name="sort"]:checked')?.value ?? "alias";

  gridItems.sort((a, b) => {
    let itemA = String(sorting).startsWith("data-")
      ? a.dataset[sorting.slice(5)]
      : a.querySelector("." + sorting).textContent;
    let itemB = String(sorting).startsWith("data-")
      ? b.dataset[sorting.slice(5)]
      : b.querySelector("." + sorting).textContent;
    if (sorting == "data-lastseen") [itemA, itemB] = [itemB, itemA];
    if (grouping) {
      let groupA = String(grouping).startsWith("data-")
        ? a.dataset[grouping.slice(5)]
        : a.querySelector("." + grouping).textContent;
      let groupB = String(grouping).startsWith("data-")
        ? b.dataset[grouping.slice(5)]
        : b.querySelector("." + grouping).textContent;
      if (groupA !== groupB) {
  ...
  gridItems.forEach((item) => {
    if (grouping) {
      const group = String(grouping).startsWith("data-")
        ? item.dataset[grouping.slice(5)] || ""
        : item.querySelector("." + grouping).textContent || "";
    ...
    let show = true;
    if (
      $('input[name="filter"][value="remote"]').checked &&
      item.dataset.isexternal == "false"
    )
      show = false;
    if (
      $('input[name="filter"][value="local"]').checked &&
      item.dataset.isexternal == "true"
    )
      show = false;
    if (
      $('input[name="filter"][value="inactive"]').checked &&
      item.querySelector(".warningicon").style.display != "inline-block"
    )
      show = false;
    if (
      $('input[name="filter"][value="pending"]').checked &&
      !item.classList.contains("tagpending")
    )
      show = false;
...
async function getTagtype(hwtype) {
  ...
  try {
    ...
    const response = await fetch(
      "/tagtypes/" +
        hwtype.toString(16).padStart(2, "0").toUpperCase() +
        ".json"
    );
    if (!response.ok) {
      let data = {
        name: "unknown id " + hwtype,
        width: 0,
        height: 0,
        bpp: 0,
        rotatebuffer: 0,
...
$("#taglist").addEventListener("contextmenu", (e) => {
  ...
  if (clickedGridItem) {
    ...
    contextMenuOptions.push({ id: "del", label: "Delete tag from list" });
